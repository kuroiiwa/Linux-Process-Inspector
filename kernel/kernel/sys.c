...
/*
 * Copy process tree data within kernel-space
 */
    ...
    int num_p, int num_tab)
{
  ...
  /* Set the process tree indentation */
...
/*
 * Traverse the process tree in DFS order
 */
...
SYSCALL_DEFINE2(ptree, struct prinfo __user *, buf, int __user *, nr)
{
  ...
  /* Obtain oldest ancestor from the tree (excluding init) */
  ...
  /* Lock the process tree before traversing it */
  read_lock(&tasklist_lock);
  traverse_prc(task, task_k, &num_p, *numr, 0);
  read_unlock(&tasklist_lock);

  /* Copy process tree data back to the user */
